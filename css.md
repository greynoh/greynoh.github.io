# CSS 기본이해하기
> css랑 html문서의 구조형태를 디자인으로 변화시키는것(꾸며주는것)
...
1. 선택자
 ...
 - 자신 선택자 : `div>ul>li`, `.container>ul>li.a{}`
 - 속성 선택자 : 'a[href="#"], `a{target="_blank"}{}` , `a[href^="http"]{}`
2. 공간의 이해
 - margin : 자신의 주변의 공간을 주는것
 - padding : 자신이 커지는것 (커지는 크기에는 어떤한 요소를 담을 수 없다.)
 - border : 외곽선을 주는것 (부피o, 크기o)
 -outline : 외곽선을 주는것 (크기만 존재, 부분을 바꿀수 없다., - 접근성 사용시 포커스 처리될때 사용)
3. RGBA, webRGB, HSLA
 ...
 - HSLA: hue(색상), saturation(채도), light(명도), alpha(투명도)
    + 먼셀 색입체
		...
		+ saturation(0~100%): 0%(무채색), 100%(흰색)
		+ light(0~100%) : 0(검정), 100%(흰색)
		+ hlsa (hue, saturation, light, alpha) => hlsa(30, 50, 50, 1)
		+ 색상 읽는 순서: a-l-s-a  순으로 읽는다
4. float 기능 이해
	- float:left | right;
	- clear		